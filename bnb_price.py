...
def get_bnb_price_message():
    """
    Fetches the current and yesterday's Binance Coin (BNB) prices and generates a message.
    Also retrieves the lowest and highest prices in the last 2.5 months.
    :return: A string message with the BNB price and percentage change.
    """

    def get_bnb_price_now():
    ...
    def get_bnb_price_on_date(date):
        url = f"https://api.coingecko.com/api/v3/coins/binancecoin/history"
        params = {"date": date}
        response = requests.get(url, params=params)
        response.raise_for_status()
        data = response.json()
    ...
    try:
        # Current price
        price_now = get_bnb_price_now()

        # Yesterday's price
        yesterday_date = datetime.now() - timedelta(days=1)
        yesterday_str = yesterday_date.strftime("%d-%m-%Y")
        yesterday_formatted = yesterday_date.strftime("%b %d, %Y")
        price_yesterday = get_bnb_price_on_date(yesterday_str)

        # Price range
        lowest_price, highest_price = get_bnb_price_range()
        ...
        if price_yesterday:
            ...
            message = (
                f"Binance Coin (BNB) price is now at ${price_now:.2f}. "
            ...
            message = "Binance Coin price data for yesterday is unavailable."

        return message, lowest_price, highest_price
